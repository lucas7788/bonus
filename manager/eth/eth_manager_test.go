...
func TestEthManager_ComputeSum(t *testing.T) {
	res := new(big.Int).Div(config.DEFAULT_GAS_PRICE, new(big.Int).SetUint64(uint64(10)))
	assert.Equal(t, res, config.OneGwei)
}
...
func TestEthManager_NewWithdrawTx(t *testing.T) {
	...
	res := new(big.Int)
	res = res.Div(config.DEFAULT_GAS_PRICE, gwei)
	fmt.Println(res)
	eth := &config.Eth{
...
func TestEthManager_EstimateFee(t *testing.T) {
	...
	fee, err := manager.EstimateFee(config.ERC20, 1)
	if err != nil {
